...
public class RatebookTest {
    ...
    @Rule
    public ActivityTestRule<Ratebook> activityTestRule = new ActivityTestRule<>(Ratebook.class);
    ...
    public void logingpback(){
        try {
            runOnUiThread(() -> {
                activityTestRule.getActivity().findViewById(R.id.ratee).performClick();
            });
        } catch (Throwable throwable) {
            throwable.printStackTrace();
        }
